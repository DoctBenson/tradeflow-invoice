...
function App() {
  ...
  function newInvoice() {
    if (window.confirm('Start a new invoice? Current data will be cleared.')) {
      const saved = localStorage.getItem('invoiceCount')
      const count = saved ? parseInt(saved) + 1 : 1
      localStorage.setItem('invoiceCount', count)
      setInvoiceNumber(`INV-2026-${String(count).padStart(3, '0')}`)
      setLineItems([{ id: 1, description: 'New Product', qty: 1, price: 0 }])
  ...
  return (
    <>
      ...
      <button onClick={newInvoice} className="new-invoice-btn">
        + New Invoice
      </button>
